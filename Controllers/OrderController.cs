...
namespace Cake.Controllers
{
    ...
    public class OrderController : Controller
    {
        ...
        public ActionResult Ilan(string redirect)
        {
            ...
            Session["orderBakery"] = dc.DBQueryGetter("select * from Bakeries where BakeryID = '" + new Guid(ilan["BakeryID"].ToString()) + "'").Rows[0];
        ...
        {
            Session["OrderBakery"] = dc.DBQueryGetter("select * from Bakeries where BakeryID = '" + new Guid(profileId) + "'");
        ...
        public void checkBakery()
        {
            ...
            for (int i = 0; i < cart.Count; i++)
            {
                ...
                {
                    Session["orderBakery"] = dc.DBQueryGetter("select * from Bakeries where BakeryID='" + new Guid(cart[i]["BakeryID"].ToString()) + "'").Rows[0];
                    bakeryFound = true;
                }
                if (cart[i]["Category"].ToString() != "Parti Malzemeleri") { isOnlyParty = false; }
            }

            if (isOnlyParty) { Session["orderBakery"] = dc.DBQueryGetter("select * from Bakeries where BakeryID = '98E94E0A-C764-4F99-A2F1-F4915631A891'").Rows[0]; bakeryFound = true; }
        ...
        public ActionResult SelectAddress(String idx)
        {
            if (idx != "subeTeslim")
            {
                ...
                {
                    DataRow bakery = dc.DBQueryGetter("select * from Bakeries where BakeryID = '" + ((DataRow)Session["orderBakery"])["BakeryID"] + "'").Rows[0];
                    bool isOk = false;
        ...
        {
            Session["orderBakery"] = dc.DBQueryGetter("select * from Bakeries where BakeryID = '" + new Guid(id) + "'").Rows[0];
            List<DataRow> cart = (List<DataRow>)Session["cart"];
        ...
        public String CalculateDesignPrice(DataRow design, String bakeryID)
        {
            ...
            DataRow bakery = dc.DBQueryGetter("select * from Bakeries where BakeryID = '" + new Guid(bakeryID) + "'").Rows[0];
            String basePrice = "";
        ...
        public void ArrangeCart()
        {
            ...
            for (int i = 0; i < cartProducts.Count; i++)
            {
                ...
                {
                    bakeries.Add(dc.MemoryCacheByQuery("select * from Bakeries where BakeryID = '" + cartProducts[i]["BakeryID"].ToString().ToUpper() + "'").Rows[0]);
                }
            }
        ...
        public ActionResult AddProductComment(FormCollection form)
        {
            ...
            else
            {
                ...
                DataRow bakery = dc.DBQueryGetter("select * from Bakeries where BakeryID = '" + bakeryID + "'").Rows[0];
        ...
        public ActionResult FaveProduct(Guid ProductID, Guid BakeryID)
        {
            ...
            if (Session["userinfo"] != null)
            {
                ...
                else
                {
                    ...
                    DataRow bakery = dc.DBQueryGetter("select * from Bakeries where BakeryID = '" + dr["BakeryID"] + "'").Rows[0];
        ...
        public void PastanelerHelper(string sehir_pastaneler, string ilce_pastaneler, string semt_pastaneler)
        {
            ...
            if (formFilled)
            {
                ...
                DataTable pastane_table = dc.DBQueryGetter("select * from Bakeries where " + query + "");
            ...
            else
            {
                if (Session["orderBakery"] != null)
                {
                    ...
                    DataTable pastane_table = dc.DBQueryGetter("select * from Bakeries where BakeryID = '" + bakery["BakeryID"] + "'");
                    ViewData["pastaneler"] = pastane_table;
                    ViewData["locationDetected"] = true;
                }
                else
                {
                    ...
                    ViewData["pastaneler"] = dc.DBQueryGetter("select * from Bakeries");
                }
                ...
                //    {
                //        ViewData["locationDetected"] = true;
                //        DataTable pastane_table = dc.DBQueryGetter("select * from Bakeries where (Province = '" + sehir_pastaneler + "' and District = '" + ilce_pastaneler + "')");
                //        ViewData["pastaneler"] = pastane_table;
                //        ViewData["fillSelects"] = new String[] { sehir_pastaneler, ilce_pastaneler, semt_pastaneler };
                //    }
