...
namespace Cake.Controllers
{
    public class BakeryController : Controller
    {
        ...
        public ActionResult Product(String ProductSelect, String ProductCount, String bakeryName, String category, String productName)
        {
            ...
            else
            {
                ...
                if (string.IsNullOrEmpty(bakeryName) == false)
                {
                    ...
                    DataTable bakeryinfo = dc.DBQueryGetter("select * from Bakeries where Name like N'%" + bakeryName + "%'");
        ...
        public void ProductHelper(DataTable dt, Guid ProductID, String ProductSelect, String ProductCount)
        {
            ...
            ViewData["ProductBakery"] = dc.MemoryCacheByQuery("select * from Bakeries where BakeryID = '" + ((DataRow)ViewData["ProductInfo"])["BakeryID"].ToString().ToUpper() + "'").Rows[0];

            bool isFaved = false;
            if (Session["userinfo"] != null) { isFaved = dc.MemoryCacheByQuery("select * from Fav where ProductID = '" + ProductID + "' and UserID = '" + ((DataRow)Session["userinfo"])["UserID"] + "'").Rows.Count > 0; }
