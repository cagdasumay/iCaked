...
namespace Cake.Controllers
{
    public class UtilityController : Controller
    {
        ...
        public String[] bakeryMarketFooter(String cat)
        {
            ...
            if (cat == "Yaş Pasta")
            {
                title = "Enfes Lezzetli ve Taze Yaş Pastalar iCaked.com’da.";
        ...
        public String productCatCorrector(DataRow product)
        {
            ...
            {
                if (String.IsNullOrEmpty(product["SizeOptions"].ToString()) == false) { result = product["SizeOptions"].ToString().Split(' ')[0] + " kişilik"; }
                else if (String.IsNullOrEmpty(product["Gram"].ToString()) == false) { result = product["Gram"].ToString().Split(' ')[0] + " gram"; }
                else if (String.IsNullOrEmpty(product["Number"].ToString()) == false) { result = product["Number"].ToString().Split(' ')[0] + " adet"; }
            }
            else
            {
                if (String.IsNullOrEmpty(product["SizeOptions"].ToString()) == false) { result = product["Select"].ToString().Split(' ')[0] + " kişilik"; }
                else if (String.IsNullOrEmpty(product["Gram"].ToString()) == false) { result = product["Select"].ToString().Split(' ')[0] + " gram"; }
                else if (String.IsNullOrEmpty(product["Number"].ToString()) == false) { result = product["Select"].ToString().Split(' ')[0] + " adet"; }
            }
            return result;
        }  
