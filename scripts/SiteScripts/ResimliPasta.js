...
function LoadDesign(id) {
    $("body").css("cursor", "progress");
    setTimeout(function () { $(".cakeStyle").removeClass("disabled-link"); $("body").css("cursor", "default"); }, 10000);
    if (designExists) { deleteDesign(); }
    designExists = true;
    $.ajax({
        ...
        success: function (result) {
            ...
            $.when(pasta.load(design[0].JsonStr)).then(function () {
                $("#canvas-loader").fadeOut(300);
                pasta.setProperties("cake-0", { scale: getCakeDimension($("#editor-cakeSize-1").val()) });
            });
            ...
            $("#canvas-upper,#canvas-container2").removeClass("disabled-link3 disabled-link"); $("#canvas-sizer").unbind("click");
            // addOGMetaTag(id);
            $("#editor-description-div").show();
            $("body").css("cursor", "default"); $(".cakeStyle").removeClass("disabled-link");
            editorStep(2);
            var textureUrl = "/Images/Site/decal_upload2.png";
            decal_idx++;
...
function saveCakeContinue() {
    ...
    function sendSaveData() {
        if (addToCart == false) {
            var content = "<form id='canvasSaveForm' action='/Object/SaveCake' method='post' style='display:none;'>" +
...
function objectGetter(name, designObject) {
    ...
    //if (name != "cake-1001") {
        ...
        if (name == "plate-0") { $("#deleteCopyObject-div").hide(); } else { $("#deleteCopyObject-div").show(); }
...
function uploadUserDecal() {
    ...
    setTimeout(function () { $(".objectTextures").trigger("click"); }, 500);
}

/******************** Editor Functions ********************/

function editorLocationSelect(val, isResimli) {
    ...
    if (isResimli != undefined) {
        if (val == "76") {
            for (i = 8; i < 62; i = i + 2) { $("#editor-cakeSize-1").append("<option value='" + i + "'>" + i + "</option>"); }
        }
        else if (val == "48") {
    ...
    $('#editor-gramaj').text('Yaklaşık ' + parseInt($("#editor-cakeSize-1").val()) * 175 + ' gr - ' + parseInt($("#editor-cakeSize-1").val()) * 200 + ' gr gelecektir.')
...
function editorStep(num) {
    if (cakeExists() == false & num != 1) {
        ...
        if (currentStep != 5) {
            $("#filter2D3D").hide(); $("#canvasNextStep").fadeIn(200); $("canvas").removeClass("disabled-link3");
        }
        ...
        if (currentStep == 1) {
            ...
            $("#decal-categories-wrapper").fadeIn(200);
        }
    ...
    if (currentStep != 5) { canvasAllowClick(); $("#pastaneSecWarning").slideUp(300); } }
}
...
function mobileArrangement() {
    ...
    $(".draggable-div2").css("max-height", $("#canvas-sizer").height() * 0.70 + "px");
...
function desktopArrangement() {
    ...
    $(".draggable-div2").css("max-height", $("#canvas-sizer").height() - 190);
...
function validatePastaNotlari() {
    ...
    if (pastaNotlariValidated == true) {
        ...
        editorBakeryAppend($('#editorCitySelect').val(), $('#editorCitySelect option:selected').text());
...
$(document).ready(function () {
    ...
    $(".add-random").click(function (event) {
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                if (typeof result.Properties.styles.color === "string")
                    color = parseInt(result.Properties.styles.color, 16);
                $.when(pasta.addRandomObject(result.EditorName, result.Source, "all", 10, { color: color })).then(function () { $("body").toggleClass("wait"); });
            }
        });
    });
    ...
    if (isMobile()) {
        if ($(window).width() < $(window).height()) {
            $("#canvas-container2").width(cWH[0] - 140);
        }
    ...
    //$("#canvas-sizer").click(function () {
    //    opacityAlert($("#editor-start-button"));
    //});

    //var firstPosX; var firstPosY; var lastPosX; var lastPosY; var isDrag = false;

    //$('canvas').on('mousedown', function (event) {
    //    $("#editorObjectPopup").fadeOut(100);
    //    firstPosX = event.pageX; firstPosY = event.pageY;
    //}).on('mouseup mouseleave', function (event) {
    //    lastPosX = event.pageX; lastPosY = event.pageY;
    //    if (Math.abs(firstPosX - lastPosX) > 30 | Math.abs(firstPosY - lastPosY) > 30) { isDrag = false; } else { isDrag = true; }
    //});

    //$("canvas").click(function (event) {
    //    mouseX = event.pageX + 30; mouseY = event.pageY - 30;
    //    if ((selectedObjName != null | selectedObjName != undefined) & isDrag) {
    //        if (selectedObjName != "plate-0") {
    //            $("#editorObjectPopup").fadeIn(100);
    //            $("#editorObjectPopup").offset({ top: mouseY, left: mouseX });
    //        }
    //        else {
    //            $("#editorObjectPopup").fadeOut(100);
    //        }
    ...
    //$('html, body').animate({
    //    scrollTop: $("#editor-main-container").offset().top
    //}, 0);
